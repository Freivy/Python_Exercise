...
class Data_Genarate():
    ...
    def data_precess(self):
        ...
        with tqdm(data.groupby('transactionid')) as t:
            for key, group in t:
                dense_features = group[self.dense_feats + self.dense_norm_feats + self.sparse_onehot_feats].values
    ...
    def data_reload(self):
        dataIn_dense_reshape = np.load('./dataIn_dense_reshape.npy', allow_pickle=True)
        self.dataIn_dense_reshape = dataIn_dense_reshape.tolist()

        dataIn_sparse_reshape = np.load('./dataIn_sparse_reshape.npy', allow_pickle=True)
    ...
    def data_padding(self):
        max_len = max([len(x) for x in self.dataIn_dense_reshape])
        dataIn_dense = pad_sequences(self.dataIn_dense_reshape, maxlen= max_len, padding='post', value=0, dtype='float64')
        print('dense shape:', dataIn_dense.shape)  # shape =(num_sample, 25, 52) =(numsize,timestep,feat_dim)
        dataIn_sparse = pad_sequences(self.dataIn_sparse_reshape, maxlen= max_len, padding='post', value=0, dtype='float64')
        print('sparse shape:', dataIn_sparse.shape)  # shape =( , 25, 8) =(numsize,timestep,feat_dim)
        dataOut = pad_sequences(self.dataOut_reshape, maxlen= max_len, padding='post', value=0, dtype='float64')
        print('out shape:', dataOut.shape)  # shape =( , 25, 1) =(numsize,timestep,feat_dim)
        train_in_dense, test_in_dense, train_in_sparse, test_in_sparse, train_data_out, test_data_out = train_test_split(dataIn_dense, dataIn_sparse, dataOut, test_size=0.3, random_state=1)
